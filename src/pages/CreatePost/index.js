...
const CreatePost = () => {
  ...
  const boardName = useSelector((store) =>
    store.categories.data?.find((c) =>
      c.boards.find((b) => b.id === parseInt(id))
    )
  )?.boards.find((d) => d.id === parseInt(id)).name;
  ...
  return (
    <div className="w-100 max-width-800">
      ...
      <div className="font-size-20 mb-20">{boardName}</div>
      <div
